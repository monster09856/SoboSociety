...
from app.models import (
    Booking, BookingDurumu, ClassSession, ClassType, CreditLedger, LedgerTipi,
)
from app.services.hatalar import DersBaslamamis, KayitBulunamadi
...
async def yoklama_al(
    ...
    *,
    ...
    # Ders başlamadan yoklama alınamaz. Bu kontrol olmadan panelde yanlış
    # karta basmak KALICI hasar demekti: ders başlamadan günler önce yoklama
    # alınıp herkes `no_show` yapılabiliyordu ve geri alma yolu yok —
    # `iptal_et` `ZatenIptal` fırlatır, ledger append-only. Panelin ana
    # ekranı tek dokunuşla yoklama olacağı için bu ucuz kontrol şart.
    if now < oturum.baslangic:
        raise DersBaslamamis("Ders henüz başlamadı, yoklama alınamaz")
    ...
    for booking_id, member_id in gelmeyenler:
        # Ceza satırı, kredinin düşüldüğü paketle aynı pakete yazılmalı —
        # burada yeni bir paket seçimi yapmak yanlış olur. Kaynak,
        # bu rezervasyonun BOOKING satırıdır.
        ...
        kaynak_paket_id = kaynak.scalar_one_or_none()
        ...
        await hareket_ekle(
            ...
            member_package_id=kaynak_paket_id,
